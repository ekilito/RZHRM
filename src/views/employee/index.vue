<template>
  <div class="container">
    ...
    <el-dialog :visible.sync="showRoleDialog" title="分配角色">
      <!-- 弹层内容 -->
      <!-- checkbox -->
      <el-checkbox-group v-model="roleIds"> <!-- 双向绑定数据 -->
        <!-- 放置n个的checkbox  要执行checkbox的存储值  :label="item.id"-->
        <el-checkbox
...
import FileSaver from 'file-saver' // 引入保存文件的包
...
export default {
  ...
  methods: {
    ...
    async btnRole(id) {
      ...
      // 记录当前点击的id 因为后面 确定取消要存取给对应的用户
      this.currentUserId = id // 记录id
      const { roleIds } = await getEmployeeDetail(id)
      this.roleIds = roleIds // 把用户拥有的角色赋值给数据的双向绑定
      this.showRoleDialog = true // 获取数据太慢了 调整顺序  先获取接口
    },
    // 点击角色的确定
    async  btnRoleOK() {
      await assignRole({
